...
def page_dashboard():
    ...
    if uploaded:
        with st.spinner("Parsing resume…"):
            from utils.resume_parser import parse_resume, extract_skills, extract_education, extract_experience, compute_resume_score
    ...
    with g1:
        ...
        missing = gap.get('missing', ['Machine Learning','SQL','Deep Learning','Power BI','Data Visualization'])[:5]
        missing_rows = "".join(f'<div style="color:#ef4444;font-size:0.83rem;padding:4px 0;border-bottom:1px solid #1e293b;">• {s}</div>' for s in missing)
        st.markdown(f"""
        <div class="dash-card">
            <div class="dash-card-title">📊 Skill Gap Report</div>
            {missing_rows}
            <div style="margin-top:10px;background:rgba(37,99,235,0.1);border:1px solid rgba(37,99,235,0.3);border-radius:8px;padding:8px 10px;font-size:0.78rem;color:#60a5fa;">
    ...
    with g2:
        ...
        jobs_rows = "".join(f'<div style="font-size:0.82rem;color:#cbd5e1;padding:4px 0;border-bottom:1px solid #1e293b;">› {j}</div>' for j in rec_jobs)
        st.markdown(f"""
        <div class="dash-card">
            <div class="dash-card-title">🎯 Recommended Domain</div>
            <div style="display:inline-block;background:#10b981;color:#fff;border-radius:8px;padding:4px 14px;font-size:0.82rem;font-weight:700;margin-bottom:12px;">{domain}</div>
    ...
    with g3:
        ...
        exams_rows = "".join(f'<div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid #1e293b;font-size:0.83rem;color:#cbd5e1;">{ic} {ex}</div>' for ic,ex in exams)
        st.markdown(f"""
        <div class="dash-card">
    ...
    with g4:
        insights = [
            ("💰","Average Salary","₹12 LPA","#10b981"),
            ("📈","Demand Level","High","#10b981"),
            ("🚀","Growth Rate","29%","#10b981"),
            ("💼","Job Openings","5000+","#f59e0b"),
        ]
        rows = "".join(f'<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #1e293b;"><span style="font-size:0.82rem;color:#94a3b8;">{ic} {lbl}</span><span style="font-size:0.85rem;font-weight:700;color:{c};">{val}</span></div>' for ic,lbl,val,c in insights)
        st.markdown(f"""
        <div class="dash-card">
...
def main():
    ...
    PAGE_MAP = {
        ...
        'about':           page_about,
        'career_insights': page_career_insights,
...
def page_career_insights():
    ...
    with c1:
        st.markdown(f"""
        <div class="dash-card">
            <div class="dash-card-title">📊 Market Overview — {domain}</div>
            <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #1e293b;"><span style="color:#94a3b8;font-size:0.87rem;">💰 Average Salary</span><span style="color:#10b981;font-weight:700;">₹12 LPA</span></div>
            <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #1e293b;"><span style="color:#94a3b8;font-size:0.87rem;">📈 Demand Level</span><span style="color:#10b981;font-weight:700;">Very High</span></div>
            <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #1e293b;"><span style="color:#94a3b8;font-size:0.87rem;">🚀 Growth Rate</span><span style="color:#10b981;font-weight:700;">29% YoY</span></div>
            <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #1e293b;"><span style="color:#94a3b8;font-size:0.87rem;">💼 Job Openings</span><span style="color:#f59e0b;font-weight:700;">5000+</span></div>
            <div style="display:flex;justify-content:space-between;padding:8px 0;"><span style="color:#94a3b8;font-size:0.87rem;">🎓 Entry Level Salary</span><span style="color:#60a5fa;font-weight:700;">₹6–8 LPA</span></div>
        </div>""", unsafe_allow_html=True)
    with c2:
        st.markdown("""
        <div class="dash-card">
            <div class="dash-card-title">🏢 Top Hiring Companies</div>
            <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #1e293b;font-size:0.85rem;color:#cbd5e1;">🔵 Google</div>
            <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #1e293b;font-size:0.85rem;color:#cbd5e1;">🟠 Amazon</div>
            <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #1e293b;font-size:0.85rem;color:#cbd5e1;">🔴 Microsoft</div>
...
def page_achievements():
    ...
    user = get_current_user() or {}
    badges = [
        ("🎯","First Resume Upload","Upload your first resume","#2563eb", user.get('resume_score',0) > 0),
        ("📝","Exam Starter","Take your first exam","#10b981", user.get('exams_taken',0) > 0),
        ("⭐","High Scorer","Score 80%+ on any exam","#f59e0b", user.get('highest_score',0) >= 80),
        ("💼","Job Explorer","View job recommendations","#7c3aed", True),
        ("📚","Learner","Visit Learning Resources","#ec4899", True),
        ("🏅","Profile Complete","Fill in your full profile","#06b6d4", bool(user.get('college'))),
    ]
    cols = st.columns(3)
    for i, (icon, title, desc, color, earned) in enumerate(badges):
        with cols[i % 3]:
            opacity = "1" if earned else "0.35"
            st.markdown(f"""
            <div class="dash-card" style="text-align:center;opacity:{opacity};">
                <div style="font-size:2.5rem;">{icon}</div>
                <div style="font-weight:700;color:{color};margin:6px 0 2px;font-size:0.92rem;">{title}</div>
...
def page_results():
    ...
    highest     = user.get('highest_score', 0)
    st.markdown(f"""
    <div class="dash-card">
        <div class="dash-card-title">📊 Your Exam Summary</div>
        <div style="display:flex;gap:32px;margin-top:8px;">
            <div style="text-align:center;"><div style="font-size:2rem;font-weight:800;color:#2563eb;">{exams_taken}</div><div style="font-size:0.8rem;color:#94a3b8;">Exams Taken</div></div>
            <div style="text-align:center;"><div style="font-size:2rem;font-weight:800;color:#10b981;">{highest}%</div><div style="font-size:0.8rem;color:#94a3b8;">Highest Score</div></div>
        </div>
    </div>""", unsafe_allow_html=True)
...
def page_notifications():
    ...
    notifications = [
        ("🎯","New job matches available for your profile!","2 minutes ago","#2563eb"),
        ("📊","Your resume score can be improved — check Skill Gap Report","1 hour ago","#f59e0b"),
        ("📝","TCS NQT registration deadline: 30 June 2026","3 hours ago","#ef4444"),
        ("🏆","Congratulations! You've earned the 'Job Explorer' badge","1 day ago","#10b981"),
        ("📚","New learning resource added: Python for Data Science","2 days ago","#7c3aed"),
    ]
    for icon, msg, time_ago, color in notifications:
        st.markdown(f"""
        <div class="dash-card" style="border-left:3px solid {color};">
            <div style="display:flex;gap:12px;align-items:flex-start;">
                <span style="font-size:1.4rem;">{icon}</span>
                <div style="flex:1;">
                    <div style="font-size:0.87rem;color:#e2e8f0;margin-bottom:4px;">{msg}</div>
                    <div style="font-size:0.75rem;color:#64748b;">{time_ago}</div>
                </div>
            </div>
        </div>""", unsafe_allow_html=True)
...
def page_settings():
    ...
    user = get_current_user() or {}
    st.markdown('<div class="dash-card"><div class="dash-card-title">🎨 Preferences</div>', unsafe_allow_html=True)
    st.selectbox("Target Domain", ["Data Science","Machine Learning","Web Development","Cloud & DevOps","Mobile Development","Cybersecurity","Software Engineering","UI/UX Design"], index=0, key="pref_domain")
    st.selectbox("Year of Study",  ["1st Year","2nd Year","3rd Year","4th Year","Graduate"], key="pref_year")
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('<div class="dash-card"><div class="dash-card-title">🔒 Account</div>', unsafe_allow_html=True)
